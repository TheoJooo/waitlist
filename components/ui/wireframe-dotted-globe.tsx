...
export default function RotatingEarth({
  ...
  interactive = true,
  rotationSpeed = 0.08,
  initialRotation = [28, -10],
}: RotatingEarthProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [initialLon, initialLat] = initialRotation
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      context.shadowBlur = 26 * scaleFactor
      context.stroke()
      context.restore()
    ...
    const rotation: [number, number] = [initialLon, initialLat]
    let autoRotate = true
  ...
  }, [initialLat, initialLon, interactive, rotationSpeed, width, height])
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <canvas
        ref={canvasRef}
        className={`w-full h-auto ${interactive ? "cursor-grab active:cursor-grabbing" : ""}`.trim()}
